...
function getTasks(responseData: DeletedTasksResponse) {
  return responseData.data?.tasks ?? responseData.tasks ?? [];
}

function getMeta(responseData: DeletedTasksResponse, tasks: TaskResponse[]) {
  return (
...
export function AdminDeletedTasksPage() {
  ...
  const fetchDeletedTasks = useCallback(async () => {
    try {
      ...
      const response = await axiosApiInstance.get<DeletedTasksResponse>(
        "/tasks/deleted",
        {
          params: {
            page,
      ...
      const nextTasks = getTasks(response.data);
      const nextMeta = getMeta(response.data, nextTasks);
  ...
  return (
    <DashboardLayout role="admin">
      <PageHeader
        title="Deleted Tasks"
        description="Admin-only page for soft deleted tasks. Edit or restore deleted tasks."
      />
      ...
      {isLoading ? (
        ...
        <div className="rounded-xl border border-dashed bg-card p-10 text-center">
          <h3 className="font-semibold">No deleted tasks</h3>
          <p className="mt-1 text-sm text-muted-foreground">
            Deleted tasks will appear here after admin or users delete tasks.
          </p>
        </div>
      ) : (
